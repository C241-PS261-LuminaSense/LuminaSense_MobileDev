...
private fun validateText(text: String, validationType: ValidationType, passwordToMatch: String?): String? {
    return when (validationType) {
        ValidationType.IpAddress -> validateIPv4Address(text)
...
private fun validateIPv4Address(text: String): String? {
    val ipv4Pattern = Regex(
        "^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$"
    )
    return if (ipv4Pattern.matches(text)) null else "Invalid IPv4 address"
}
